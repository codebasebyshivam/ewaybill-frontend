...
const FlowSteps = ({
  ...
  return (
    <section
      className={`hidden md:block mb-20 transform transition-all duration-1000 delay-700 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}
    >
        ...
        {flowSteps.map((step, index) => {
          ...
          return (
            <div key={index} className="flex flex-col items-center relative">
              <div
                className={`relative w-16 h-16 rounded-full flex items-center justify-center mb-3 transition-all duration-500 ${
                  isActive
              ...
              <span
                className={`text-sm font-medium transition-colors ${isActive ? 'text-teal-600' : 'text-gray-600'}`}
              >
                {step.name}
              </span>
              ...
              {index < flowSteps.length - 1 && (
                <div
                  className={`absolute top-8 left-20 w-16 h-0.5 transition-all duration-500 ${
                    isCompleted
