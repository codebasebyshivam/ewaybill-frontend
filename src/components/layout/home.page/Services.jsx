...
const Services = ({ isLoaded, activeService, setActiveService }) => {
  return (
    <section className={`transition-all duration-1000 delay-900 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
      <div className="text-center mb-12">
        <h3 className="text-3xl md:text-4xl font-bold text-gray-800 mb-4">Complete Fleet Ecosystem</h3>
          ...
          return (
            <div
              ...
              className={`group relative bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-lg border border-white/50 cursor-pointer transform transition-all duration-500 ${
                isActive ? 'scale-105 shadow-2xl ring-4 ring-teal-500/20' : 'hover:scale-102'
              }`}
              onMouseEnter={() => setActiveService(service.id)}
              onMouseLeave={() => setActiveService(null)}
            >
              <div className="flex items-start space-x-6">
                <div className={`p-4 rounded-2xl bg-gradient-to-br ${service.color} ${isActive ? 'scale-110 shadow-lg' : ''} transition-all`}>
              ...
              <div className={`absolute inset-0 bg-gradient-to-br ${service.color}/10 rounded-3xl transition-all duration-500 ${isActive ? 'opacity-100' : 'opacity-0'}`} />
            </div>
          );
        })}
      </div>
