...
export const MenuItem = memo(({ item, isBottom = false }) => {
  ...
  const isConceptuallyActive = hasSubItems
    ...
    : location.pathname === item.path ||
      (item.id === 'dashboard' && location.pathname === '/profile'); // Adjusted for dashboard path
  ...
  return (
    <div className="mb-1">
      ...
      >
        <div className="flex items-center space-x-3">
          <Icon
            size={20}
            className={
              (location.pathname === item.path && !hasSubItems) ||
              isConceptuallyActive
                ? 'text-white'
                : 'text-gray-500 group-hover:text-teal-700'
            }
          />{' '}
          {/* Styling for icon */}
      ...
      {hasSubItems &&
        (isExpanded || isConceptuallyActive) && ( // Render sub-items if expanded OR conceptually active
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500 truncate">
                {`GSTIN - ${user?.gst}` || 'GSTIN - XXXXXXX...'}{' '}
                {/* Assuming email or another identifier exists */}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="px-4 py-6 border-t border-gray-200 flex flex-col space-y-4 bg-background-grey">
          ...
          <div className="mt-auto pt-4 border-t border-gray-100">
            <div
              ...
              className="flex items-center px-3 py-2.5 rounded-lg  cursor-pointer bg-action-button-gradient hover:bg-t1 text-white transition-all duration-200"
            >
